...
param sql_server_outputs_name string

param sql_server_outputs_sqlserveradminname string

param principalId string

param principalName string

resource sql_server 'Microsoft.Sql/servers@2023-08-01' existing = {
  name: sql_server_outputs_name
}

resource sqlServerAdmin 'Microsoft.ManagedIdentity/userAssignedIdentities@2024-11-30' existing = {
  name: sql_server_outputs_sqlserveradminname
}
...
resource script_sql_server_database 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  name: take('script-${uniqueString('sql_server', principalName, 'database', resourceGroup().id)}', 24)
  location: location
  identity: {
    type: 'UserAssigned'
  ...
  properties: {
    ...
    environmentVariables: [
      ...
      {
        name: 'DBSERVER'
        value: sql_server.properties.fullyQualifiedDomainName
      }
      {
        name: 'PRINCIPALTYPE'
